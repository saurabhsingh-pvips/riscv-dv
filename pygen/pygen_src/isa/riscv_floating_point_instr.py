...
class riscv_floating_point_instr(riscv_instr):
    ...
    def update_src_regs(self, operands = []):
        ...
        if self.format.name == "I_FORMAT":
            if self.has_fs1:
                ...
                self.fs1_value = self.get_gpr_state(operands[1])
            elif self.has_rs1:
            ...
            self.fs2_value = self.get_gpr_state(operands[0])
            self.rs1 = self.get_gpr(operands[1])
            self.rs1_value = self.get_gpr_state(operands[1])
            self.imm = get_val(operands[2])
        elif self.format.name == "R_FORMAT":
            if len(operands) == 2 and self.instr_name.name in ["FSGNJ_S", "FSGNJX_S",
                                                               "FSGNJN_S", "FSGNJ_D",
                                                               "FSGNJX_D", "FSGNJN_D"]:
            ...
            self.fs2_value = self.get_gpr_state(operands[2])
            self.fs3 = self.get_fpr(operands[3])
            self.fs3_value = self.get_gpr_state(operands[3])
        else:
    ...
    def update_dst_regs(self, reg_name, val_str):
        ...
        if self.has_fd:
            self.fd = self.get_fpr(reg_name)
            self.fd_value = self.get_gpr_state(reg_name)
        elif self.has_rd:
            self.rd = self.get_gpr(reg_name)
            self.rd_value = self.get_gpr_state(reg_name)

    def get_fpr(self, reg_name):
        return riscv_fpr_t[reg_name.upper()]
