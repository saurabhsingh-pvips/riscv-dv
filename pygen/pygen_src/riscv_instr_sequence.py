...
# from riscv_instr_stream import *
...
#    else 
#      allow_branch = 1
#    allow_branch &= not self.cfg.no_branch_jump
    problem = constraint.Problem(constraint.MinConflictsSolver())
    problem.addVariable(stk_len, range(self.cfg.min_stack_len_per_program, self.cfg.min_stack_len_per_program))

    def cons_stk_len(stk_len):
      range_lst = list(range(self.cfg.min_stack_len_per_program, self.cfg.min_stack_len_per_program))
      if (stk_len in range_lst) and (stk_len % riscv_core_setting.XLEN/8):
...
#    self.instr_stack_enter.cfg = cfg
  ...
  def gen_stack_exit_instr(self):
    self.instr_stack_exit.cfg = self.cfg
