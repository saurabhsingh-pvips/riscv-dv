...
class riscv_load_store_base_instr_stream(riscv_mem_access_stream):
    ...
    def addr_c(self):
        ...
        # self.values = self.data_page[self.data_page_id].values()
    ...
    def gen_load_store_instr(self):
        ...
        if((self.rs1_reg in [riscv_reg_t.S0, riscv_reg_t.S1, riscv_reg_t.A0, riscv_reg_t.A1,
                             riscv_reg_t.A2, riscv_reg_t.A3, riscv_reg_t.A4,
                             riscv_reg_t.A5, riscv_reg_t.SP]) and  # TODO
                not(cfg.disable_compressed_instr)):
            enable_compressed_load_store = 0
        for i in range(len(self.addr)):
            # Assign the allowed load/store instructions based on address alignment
            # This is done separately rather than a constraint to improve the
            # randomization performance.
            allowed_instr.extend(
                [riscv_instr_name_t.LB.name, riscv_instr_name_t.LBU.name,
                    riscv_instr_name_t.SB.name])
            if(not cfg.enable_unaligned_load_store):
                if(self.addr[i] == 0):
                    allowed_instr.extend(
                        [riscv_instr_name_t.LH.name, riscv_instr_name_t.LHU.name,
                            riscv_instr_name_t.SH.name])
                if(self.addr[i] % 4 == 0):
                ...
                if((rcs.XLEN >= 64) and (self.addr[i] % 0 == 0)):
                    ...
                    if(rcs.XLEN >= 64):
                        allowed_instr.extend(
                            [riscv_instr_name_t.LWU.name, riscv_instr_name_t.LD.name,
                            ...
                            if(self.rs1_reg == riscv_reg_t.SP):
                                allowed_instr.extend(
                                    [riscv_instr_name_t.C_LWSP.name,
                                        riscv_instr_name_t.C_SWSP.name])
                            else:
                                allowed_instr.extend(
                                    [riscv_instr_name_t.C_LD.name, riscv_instr_name_t.C_SD.name])
...
# TODO : class riscv_hazard_instr_stream

# Use a small set of address to create various load/store hazard sequence
# This instruction stream focus more on hazard handling of load store unit.
# TODO : class riscv_load_store_hazard_instr_stream
...
class riscv_multi_page_load_store_instr_stream(riscv_mem_access_stream):
    def __init__(self):
        ...
        self.num_of_instr_stream = vsc.rand_uint32_t(0)
        self.data_page_id = vsc.rand_list_t(vsc.uint8_t())
    ...
    def default_c(self):
        ...
        with vsc.foreach(self.rs1_reg, idx = True) as i:
            ...
            self.rs1_reg[i] != riscv_reg_t.ZERO

    @vsc.constraint
    def page_c(self):
        # TODO : solve num_of_instr_stream before data_page_id
        # self.num_of_instr_stream.inside(vsc.rangelist(1, self.max_data_page_id))
    ...
    def reasonable_c(self):
        self.num_of_instr_stream.inside(vsc.rangelist((2,8)))'''

    # Generate each load/store seq, and mix them together
    def post_randomize(self):
        self.load_store_instr_stream = [] * self.num_of_instr_stream
        for i in range(len(self.load_store_instr_stream)):
            ...
            self.load_store_instr_stream[i].sp_c.constraint_mode(0)
            # Make sure each load/store sequence doesn't override the rs1 of other sequences.
            with vsc.foreach(self.rs1_reg, idx = True) as j:
                if i != j:
                    self.load_store_instr_stream[i].reserved_rd.append(self.rs1_reg[j])
            try:
                with self.load_store_instr_stream[i].randomize_with():
                    logging.info("TODO")
                    # rs1_reg == local::rs1_reg[i]
                    # self.data_page_id == local::data_page_id[i]
            except Exception:
                logging.critical("Cannot randomize load/store instruction")
                sys.exit(1)
            # Mix the instruction stream of different page access, this could trigger the \
            # scenario of frequent data TLB switch
            if i == 0:
...
class riscv_mem_region_stress_test(riscv_multi_page_load_store_instr_stream):
    ...
    @vsc.constraint
    def page_c(self):
        # self.num_of_instr_stream.inside(vsc.rangelist((2, 5)))
        with vsc.foreach(self.data_page_id, idx = True) as i:
            if i > 0:
