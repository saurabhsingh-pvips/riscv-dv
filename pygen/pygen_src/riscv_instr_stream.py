...
'''
  ...
  def insert_instr_stream(self, new_instr, idx=-1, replace=0):
    ...
    self.instr_list = self.instr_list[0:idx] + new_instr + self.instr_list[idx + 1:current_instr_cnt]
...
class riscv_rand_instr_stream(riscv_instr_stream):
    ...
    def setup_allowed_instr(self, no_branch=False, bit no_load_store=True):
        ...
        if no_branch == 0:
            self.allowed_instr.append(riscv_instr.instr_category[riscv_instr_category_t.BRANCH.name])
        if no_load_store == 0:
    ...
    def randomize_instr(self, instr, is_in_debug=False, disable_dist=False):
        exclude_instr = []
        if ((riscv_reg_t.SP in self.reserved_rd) or (riscv_reg_t.SP in cfg.reserved_regs)) or
        not (riscv_reg_t.SP in self.avail_regs)):
