    ...
    def gen_trap_handler_section(self, hart, mode, cause, tvec,
        ...
        if cfg.mtvec_mode == mtvec_mode_t.VECTORED:
            ...
            if cfg.check_xstatus:
                instr.append("csrr x{}, {} # {}".format(cfg.gpr[0], hex(status), status.name))
        ...
        if cfg.mtvec_mode == mtvec_mode_t.VECTORED:
            self.gen_interrupt_vector_table(hart, mode, status, cause, ie, ip, scratch, instr)
        self.gen_signature_handshake(instr, signature_type_t.CORE_STATUS, core_status_t.HANDLING_EXCEPTION)
        # The trap is caused by an exception, read back xCAUSE, xEPC to see if these
        # CSR values are set properly. The checking is done by comparing against the log
        # generated by ISA simulator (spike).
        instr.append("csrr x{}, 0x{} # {}".format(cfg.gpr[0], epc, epc.name))
        instr.append("csrr x{}, 0x{} # {}".format(cfg.gpr[0], cause, cause.name))

        # Illegal instruction exception
        instr.append("li x{}, {} # ILLEGAL_INSTRUCTION".format(
            cfg.gpr[1], hex(exception_cause_t.ILLEGAL_INSTRUCTION)))
        instr.append("beq x{}, x{}, {}illegal_instr_handler".format(cfg.gpr[0], cfg.gpr[1], pkg_ins.hart_prefix(hart)))
        # Skip checking tval for illegal instruction as it's implementation specific
        instr.append("csrr x{}, {} # {}".format(cfg.gpr[1], hex(tval), tval.name))
        # use JALR to jump to test_done.
        instr.append("1: la x{}, test_done".format(cfg.scratch_reg))
        instr.append("jalr x1, x{}, 0".format(cfg.scratch_reg))
