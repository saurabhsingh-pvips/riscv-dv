...
class riscv_asm_program_gen:
    ...
    def gen_trap_handler_section(self, hart, mode, cause, tvec,
        ...
        instr.append(pkg_ins.format_string("csrr x{}, {} # {}".format(cfg.gpr[0], hex(epc), epc.name)))
        instr.append(pkg_ins.format_string(
            "csrr x{}, {} # {}".format(cfg.gpr[0], hex(cause), cause.name)))
        instr.append(pkg_ins.format_string("li x{}, {} #ECALL_MMODE".format(cfg.gpr[1], hex(exception_cause_t.ECALL_MMODE))))
        instr.append(pkg_ins.format_string("beq x{}, x{}, {}ecall_handler".format(cfg.gpr[0], cfg.gpr[1], pkg_ins.hart_prefix(hart))))
        # Illegal instruction exception
        instr.append(pkg_ins.format_string("li x{}, {} # ILLEGAL_INSTRUCTION".format(
