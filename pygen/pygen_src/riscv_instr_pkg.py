...
# mport riscv_page_table_exception_cfg
...
def MAX_CALLSTACK_DEPTH():
    return 20


def MAX_SUB_PROGRAM_CNT():
    return 20


def MAX_CALL_PER_FUNC():
    return 5


def indent():
    lable_str_len = LABEL_STR_LEN()
    return lable_str_len
...
def pop_gpr_from_kernel_stack(status, scratch, mprv, sp, tp, instr=[]):
    ...
    instr.push_back(print("addi", sp, sp, 31 * (XLEN / 8)))
    # if(scratch inside {implemented_csr}):
