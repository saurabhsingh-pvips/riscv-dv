...
from pygen_src.riscv_core_setting import *  # NOQA
...
class riscv_instr_gen_config:
    def __init__(self):
        ...
        self.mstatus = BitArray(bin(0b0), length=XLEN - 1)
        self.mie = BitArray(bin(0b0), length=XLEN - 1)
        self.sstatus = BitArray(bin(0b0), length=XLEN - 1)
        self.sie = BitArray(bin(0b0), length=XLEN - 1)
        self.ustatus = BitArray(bin(0b0), length=XLEN - 1)
        self.uie = BitArray(bin(0b0), length=XLEN - 1)
        self.mstatus_mprv = 0
        self.mstatus_mxr = 0
        self.mstatus_sum = 0
        self.mstatus_tvm = 0
        ...
        self.no_wfi = 1
        self.min_stack_len_per_program = 10 * (XLEN / 8)
        ...
        self.gpr_lst = [list(map(lambda gpr_i: gpr_i.name, riscv_reg_t))
                        for lst in range(5)]
        ...
        # category_dist dict initisation
    ...
    def check_setting(self):
        ...
        supported_isa_stng_lst = supported_isa
    ...
    def pre_randomize(self):
        for x in supported_privileged_mode:
