...
from pygen_src.riscv_instr_pkg import data_pattern_t, \
    mtvec_mode_t, f_rounding_mode_t, riscv_reg_t, \
    exception_cause_t, interrupt_cause_t, privileged_mode_t, \
    riscv_instr_category_t, riscv_instr_group_t
from pygen_src.target.rv32i import riscv_core_setting as stng
...
class riscv_instr_gen_config:
    def __init__(self):
        ...
        self.mstatus = BitArray(bin(0b0), length=stng.XLEN - 1)
        self.mie = BitArray(bin(0b0), length=stng.XLEN - 1)
        self.sstatus = BitArray(bin(0b0), length=stng.XLEN - 1)
        self.sie = BitArray(bin(0b0), length=stng.XLEN - 1)
        self.ustatus = BitArray(bin(0b0), length=stng.XLEN - 1)
        self.uie = BitArray(bin(0b0), length=stng.XLEN - 1)
        self.mstatus_mprv = 0
        self.mstatus_mxr = 0
        self.mstatus_sum = 0
        self.mstatus_tvm = 0
        ...
        self.no_wfi = 1
        self.min_stack_len_per_program = 10 * (stng.XLEN / 8)
        self.max_stack_len_per_program = 16 * (stng.XLEN / 8)
        self.stack_len = 5000
        ...
        self.num_of_harts = stng.NUM_HARTS
        self.tvec_alignment = 2
        self.fix_sp = 0
        self.mode_opts = "boot_mode_opts"
        self.enable_page_table_exception = 0
        self.enable_nested_interrupt = 0
        self.test_suffix = "asm_test_suffix"
        ...
        self.category_dist = dict()
        self.gpr = [list(map(lambda gpr_i: gpr_i.name, riscv_reg_t))
                    for lst in range(5)]
        ...
        self.init_privileged_mode = list(
            map(lambda mode: mode.name, privileged_mode_t))
        ...
        # category_dist dict initialsation
        self.catgry_lst = list(
            map(lambda cat_lst: cat_lst.name, riscv_instr_category_t))
        for lst_nm in self.catgry_lst:
            self.category_dist[lst_nm] = None
    ...
    def check_setting(self):
        ...
        supported_isa_stng_lst = stng.supported_isa
    ...
    def pre_randomize(self):
        for x in stng.supported_privileged_mode:
    ...
    def post_randomize(self):
        ...
        self.min_stack_len_per_program = 2 * (stng.XLEN / 8)
