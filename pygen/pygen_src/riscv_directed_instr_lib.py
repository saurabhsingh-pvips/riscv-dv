...
class riscv_int_numeric_corner_stream(riscv_directed_instr_stream):
    ...
    def __init__(self):
        ...
        self.init_val = vsc.list_t(vsc.rand_bit_t())
        self.init_instr = []
        self.int_numeric_v = vsc.rand_enum_t(int_numeric_e)
        self.init_val_type = []
    ...
    def init_val_c(self):
        ...
        # len(self.init_val_type) == self.num_of_avail_regs
        # len(self.init_val) == self.num_of_avail_regs
        self.num_of_instr in vsc.rangelist(vsc.rng(15, 30))

    @vsc.constraint
    def avail_regs_c(self):
        vsc.unique(self.avail_regs)
        for i in len(self.avail_regs):
            # self.avail_regs[i] not in cfg.reserved_regs
            self.avail_regs[i] != 0
	...
	# TODO
        ...
        for i in range(len(self.init_val_type)):
            if self.init_val_type[i] == Zero:
                self.init_val[i] = 0
            elif self.init_val_type[i] == AllOne:
                self.init_val[i] = 1
            elif self.init_val_type[i] == NegativeMax:
