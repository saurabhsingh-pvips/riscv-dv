...
class riscv_program:
    def __init__(self):
        self.program_id = vsc.rand_enum_t(program_id_t)
        self.call_stack_level = vsc.rand_uint32_t()
        self.sub_program_id = vsc.rand_list_t(vsc.enum_t(program_id_t))
    ...
    def convert2string(self):
        ...
        for i in range(len(self.sub_program_id)):
            string = "{} {}".format(string, self.sub_program_id)
        return string
...
class riscv_callstack_gen:
    def __init__(self):
        ...
        self.stack_level = vsc.rand_list_t(vsc.bit_t(11))
    ...
    def init(self, program_cnt):
        ...
        for i in range(len(program_h)):
            object_name = f"program_{i}"
            object_name = riscv_program()
            self.program_h[i].append(object_name)
