...
class riscv_instr:
    ...
    def create_instr_list(self, cfg):
        ...
        for instr_name, values in self.instr_registry.items():
            ...
            print("Category {}", self.instr_category[instr_inst.category.name])
     ...
     def get_rand_instr(self, include_instr, exclude_instr, \
        ...
        if(len(disallowed_instr) == 0):
            if(len(include_instr) > 0):
                idx = randrange(0 , len(include_instr) - 1)
                name = include_instr[idx]
            elif(len(allowed_instr > 0)):
                idx = randrange(0, len(allowed_instr) - 1)
                name = allowed_instr[idx]
            else:
                idx = randrange(0, len(self.instr_names) - 1)
    ...
    def convert2asm(self, prefix = " "):
        asm_str = type(self).__name__.format_string((type(self).__name__, type(self).__name__.MAX_INSTR_STR_LEN))

        if(self.category != "SYSTEM"):
            if(self.format in ["J_FORMAT", "U_FORMAT"]):
                asm_str = type(self).__name__.format_string((type(self).__name__, type(self).__name__.MAX_INSTR_STR_LEN))
