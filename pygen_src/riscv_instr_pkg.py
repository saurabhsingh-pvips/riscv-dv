...
def MAX_USED_VADDR_BITS():                # parameter int MAX_USED_VADDR_BITS = 30
    return 30
...
#typedef bit[11:0] riscv_csr_t;    # to do.....
...
if _VCP:
    class pmp_cfg_reg_t(Enum):   # struct packed
        l = auto()
	zero = auto()
	a = auto()          #  pmp_addr_mode_t  a;
	x = auto()
	w = auto()
	r = auto()
	addr = auto()      #bit [XLEN - 1 : 0]    addr;    
	offset = auto()    #bit [XLEN - 1 : 0]    addr;
...
#The offset from the address of <main> - automatically populated by the PMP generation routine.
    ...
    class pmp_cfg_reg_t(Enum):   # struct
        l = randint(0,)
	zero = auto()
	a = randint(0,)     # pmp_addr_mode_t a;
	x = randint(0,)
	w = randint(0,)
	r = randint(0,)
	addr = auto()
...
class vtype_t(Enum):
    ill = auto() 
    reserved = auto()                    # bit [XLEN-2:7] reserved
    vediv = auto()  
    vsew = auto() 
    vlmul = auto() 
...
#`VECTOR_INCLUDE("riscv_instr_pkg_inc_variables.sv")   # to do.....
...
def MPRV_BIT_MASK():                # parameter bit[XLEN - 1: 0] MPRV_BIT_MASK = 'h1 << 17
    return 0x1 << 17

def SUM_BIT_MASK():                 # parameter bit [XLEN - 1 : 0] SUM_BIT_MASK  = 'h1 << 18
    return 0x1 <<18

def MPP_BIT_MASK():                 # parameter bit [XLEN - 1 : 0] MPP_BIT_MASK  = 'h3 << 11
    return 0x3 << 11


def IMM25_WIDTH():                  # parameter int IMM25_WIDTH = 25
    return 25

def IMM12_WIDTH():                  # parameter int IMM12_WIDTH = 12
    return 12

def INSTR_WIDTH():                  # parameter int INSTR_WIDTH = 32;
    return 32

def DATA_WIDTH():                   # parameter int DATA_WIDTH  = 32;
    return 32


#Parameters for output assembly program formatting

def MAX_INSTR_STR_LEN():             # parameter int MAX_INSTR_STR_LEN = 11;
    return 11

def LABEL_STR_LEN():                 # parameter int LABEL_STR_LEN     = 18;
    return 18


#Parameter for program generation

def MAX_CALLSTACK_DEPTH():           # parameter int MAX_CALLSTACK_DEPTH = 20;
    return 20

def MAX_SUB_PROGRAM_CNT():           # parameter int MAX_SUB_PROGRAM_CNT = 20;
    return 20

def MAX_CALL_PER_FUNC():             # parameter int MAX_CALL_PER_FUNC   = 5;
    return 5

def indent():                        # string indent = {LABEL_STR_LEN{" "}};
    return {LABEL_STR_LEN{" "}}
...
def format_string(str, len=10):

    formatted_str = auto()

    formatted_str = {len{" "}}     # formatted_str = {len{" "}};
...
def format_data(data[], byte_per_group = 4):    #  bit [7:0] data[]  # to do...........
	
    cnt =auto()
    str = "0x"	

    for i in data[i]:                                                                 # foreach(data[i])
        if ((i % byte_per_group == 0) and (i != data.len() - 1) and (i != 0)):
            str = {str, ", 0x"}
...
def get_instr_name(str):

    instr = riscv_instr_name_t()

    instr = instr.first                       # to do .....instr.first
    
    for_loop=0
    while(for_loop>=0):                       # forever loop implemenation
                                           
        if (str.toupper() == instr.name):     # to do .......instr.name
            return instr

        if (instr == instr.last):             # to do...... instr.last
            return INVALID_INSTR

        instr = instr.next                    # to do........instr.next 
...
def push_gpr_to_kernel_stack(status, scratch, mprv, sp, tp, instr = []):    # Initializing a queue:: list as queue (instr =[]) == (instr=[$])
    store_instr = (XLEN == 32) ? "sw" : "sd"                                # Note Xstaus len = XLEN
    if (scratch inside {implemented_csr}):                                  # constraint part....
        ...
        instr.push_back(print("add ", sp, tp))	
                                                                            # If MPRV is set and MPP is S/U mode, 
                                                                            # it means the address translation and memory protection
                                                                            # for load/store instruction is the same as the mode indicated by MPP. 
                                                                            # In this case, we need to use the virtual address to access the kernel stack.

    if((status == MSTATUS) and (SATP_MODE != BARE)):
                                                                            # We temporarily use tp to check mstatus to avoid changing other GPR. 
                                                                            # The value of sp has been saved to xScratch and can be restored later.
        if(mprv):
	    instr.append(print("csrr // MSTATUS", tp, status))
            instr.append(print("srli ", tp, tp))                            # Move MPP to bit 0
            instr.append(print("andi ", tp, tp))                            # keep the MPP bits
                                                                            # Check if MPP equals to M-mode('b11)
            instr.append(print("xori ", tp, tp))
            instr.append(print("bnez ", tp))                                # Use physical address for kernel SP
                                                                            # Use virtual address for stack pointer
            instr.append(print("slli ", sp, sp, XLEN - MAX_USED_VADDR_BITS))
            #instr.append(print("srli ", sp, sp, XLEN - MAX_USED_VADDR_BITS))

                                                                            # Reserve space from kernel stack to save all 32 GPR except for x0
    instr.append(print( "1: ", sp, sp, 31 *(XLEN/8) ) )
                                                                            # Push all GPRs to kernel stack
    for i in range(0,32):
	instr.append(print(" the store_instr, sp", store_instr, i, i * (XLEN/8), sp))
...
def pop_gpr_from_kernel_stack(status, scratch, mprv, sp, tp, instr = [ ])):
    ...
    for i in range(0,32):                                                               # Pop user mode GPRs from kernel stack
        instr.append(print(load_instr, i, i * (XLEN/8), sp))
	

    instr.push_back(print("addi ", sp, sp, 31*(XLEN/8)))                                 # Restore kernel stack pointer



    if (scratch inside {implemented_csr}) :   # to do.....constraint....inside.....   
        instr.append(print("add ", tp, sp))                                              # Move SP to TP 
	instr.append(print("csrrw ", sp, scratch, sp)) 	                                 # Restore user mode stack pointer
...
class cmdline_enum_processor:  # (parameter type T = riscv_instr_group_t):      # to do............
    def get_array_values(self, cmdline_strcmdline_str, vals=[]):                # taken array as a list
	s = auto()

	casting_static = int(inst.get_arg_value(cmdline_str, s))                # void'(inst.get_arg_value(cmdline_str, s)), to do static type casting......
        print("casting_static =",casting_static)

	if(s != " "):
            pass

		#cmdline_list = []                                               # list as a queue
		# T value
		# uvm_split_string(s, ",", cmdline_list);
		# vals = new[cmdline_list.size]                                  # to do.....
